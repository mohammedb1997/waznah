...
(function () {
  ...
  // غيّر الرابط إلى دومين PocketBase ثابت عند الانتقال للإنتاج.
  var POCKETBASE_BASE_URL = 'https://armful-gumming-germless.ngrok-free.dev';
  var FETCH_TIMEOUT_MS = 6000;
  // لا نرسل ngrok-skip-browser-warning من fetch العادي لأنه يفرض CORS preflight.
  // إذا كنت مضطراً لاستخدامه، مرره عبر Proxy/BFF بدلاً من كشفه في المتصفح.
  var FETCH_HEADERS = {};
  ...
  function fetchAll(collection) {
    var all = [];
    function page(p) {
      var url = buildUrl(POCKETBASE_BASE_URL, '/api/collections/' + encodeURIComponent(collection) + '/records?page=' + p + '&perPage=200');
      return fetchWithTimeout(url, { headers: FETCH_HEADERS, credentials: 'omit', cache: 'no-store' }, FETCH_TIMEOUT_MS)
        .then(function (res) {
  ...
  function fileUrl(collection, record, filename) {
    ...
    return buildUrl(POCKETBASE_BASE_URL, '/api/files/' + encodeURIComponent(collection) + '/' + encodeURIComponent(record.id) + '/' + encodeURIComponent(filename));
  }
